...
{
    /// <summary>
    /// This part of this splitter will remove line that already exists and will split lines that are close to an existing line.
    ...
    /// <param name="existingLineStrings">The lines to test against</param>
    /// <param name="minimalMissingPartLength">The minimal length allowed to a trace that can be added</param>
    /// <param name="minimalDistanceToClosestPoint">The distance of the closest point allowed</param>
    /// <returns>a split line from the original line</returns>
    List<LineString> GetMissingLines(LineString gpxLine, IReadOnlyList<LineString> existingLineStrings, double minimalMissingPartLength, double minimalDistanceToClosestPoint);
    /// <summary>
    /// This part of the splitter if responsible for splitting a line with a self loop.
    /// It will always return lines that do not have self loop, but can be duplicate of one another
    ...
    List<LineString> SplitSelfLoops(LineString gpxLine, double minimalDistanceToClosestPoint);
