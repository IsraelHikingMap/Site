    ...
    : IReceiptValidationGateway
{
    ...
    public async Task<bool> IsEntitled(string userId)
    {
        ...
        logger.LogInformation("Is entitled with Iaptic for user: " + userId + " is: " + returnValue);
        if (returnValue)
        {
            return true;
        }
        ...
        returnValue = responseStr.Contains("\"items\":[{\"");
        logger.LogInformation("Is entitled with Revenuecat for user: " + userId + " is: " + returnValue);
        return returnValue;
    }
}