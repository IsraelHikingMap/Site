...
@REM current version is 0.6 stable from:
@REM https://graphhopper.com/public/releases/graphhopper-web-0.6.0-bin.zip
@REM http://mvnrepository.com/artifact/com.graphhopper/graphhopper-tools/0.6.0

@REM Up-to-date Graphhopper jar files can be downloaded from 
...
title 1. Update the OSM pbf, if needed, or exit
...
IF ERRORLEVEL 1 (
  ...
  title 2. Remove old databases, if exists
  @ECHO %TIME%
  ...
  title 3. Build the new graph in a temporary directory
  @ECHO %TIME%
...
IF EXIST israel-and-palestine-latest.osm-gh-new\ (
  ...
  title 4. Rename the previous database directory
  @ECHO %TIME%
  ...
  title 5. Rename the temporary database directory to expected location
  RENAME israel-and-palestine-latest.osm-gh-new israel-and-palestine-latest.osm-gh
)

tite 6. Start the server
@ECHO %TIME%
...
title 7. Remove the old database, if exists
@ECHO %TIME%
IF EXIST israel-and-palestine-latest.osm-gh-old\ (
  RMDIR /S/Q israel-and-palestine-latest.osm-gh-old
)
