...
object CarStatisticsCalculator {
    ...
    private data class ClosestRouteHit(
            ...
            /**
             * nearestPointOnLine result: properties `dist` and `index`; geometry is the projected
             * Point.
             */
    ...
    /**
     * Picks the route the driver is most likely on (perpendicular distance + heading penalty, same
     * weighting as getClosestRouteToGPSInternal in the web client), then derives remaining distance
     * by subtracting the projection's along-line position from the total line length.
     *
     * Returns null when there is no usable speed, or no route scores below the MINIMAL_DISTANCE /
     * MINIMAL_ANGLE threshold.
     */
    ...
    /**
     * Distance from the line start to the projected GPS point, in meters. Computed manually because
     * maplibre-turf's nearestPointOnLine only sets `dist` and `index` on the returned feature —
     * unlike Turf.js, it does not populate a `location` (along-line distance) property.
     */
    ...
    private fun findClosestRoute(routes: List<CarRouteData>, location: Location): ClosestRouteHit? {
        ...
        for (route in routes) {
            ...
            val projection =
                    TurfMisc.nearestPointOnLine(gpsPoint, linePoints, TurfConstants.UNIT_METERS)
