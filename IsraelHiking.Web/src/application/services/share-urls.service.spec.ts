...
import {
  AddShareUrlAction,
  RemoveShareUrlAction,
  ShareUrlsReducer,
  UpdateShareUrlAction,
  SetShareUrlsLastModifiedDateAction,
} from "../reducers/share-urls.reducer";
...
describe("Share Urls Service", () => {
  ...
  it("Should not do anything in initialization if user is not logged in", inject(
    [ShareUrlsService, HttpTestingController, Store],
    async (
  ...
  it("Should sync urls when initializing", inject(
    [ShareUrlsService, HttpTestingController, Store],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
  ...
  it("Should sync urls when initializing and update their start point if missing", inject(
    [ShareUrlsService, HttpTestingController, Store, DatabaseService],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
      store: Store,
  ...
  it("Should get display name only for the title", inject(
    [ShareUrlsService],
    (shareUrlService: ShareUrlsService) => {
      expect(
        shareUrlService.getShareUrlDisplayName({ title: "title" } as ShareUrl)
      ).toBe("title");
    }
  ));

  it("Should get display name for title and description", inject(
    [ShareUrlsService],
    (shareUrlService: ShareUrlsService) => {
      const displayName = shareUrlService.getShareUrlDisplayName({
        title: "title",
        description: "desc",
      } as ShareUrl);
  ...
  it("Should get social links for null share url", inject(
    [ShareUrlsService],
    (shareUrlService: ShareUrlsService) => {
  ...
  it("Should get social links", inject(
    [ShareUrlsService],
    (shareUrlService: ShareUrlsService) => {
      const links = shareUrlService.getShareSocialLinks({
        title: "title",
        description: "desc",
      } as ShareUrl);
  ...
  it("Should get share url from database when already cached and don't refresh it if it's not newer", inject(
    [ShareUrlsService, HttpTestingController, DatabaseService],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
      databaseService: DatabaseService
      ...
      mockBackend
        .expectOne(Urls.urls + "5/timestamp")
        .flush(new Date(0).toISOString());
  ...
  it("Should get share url from server if not cached", inject(
    [ShareUrlsService, HttpTestingController, DatabaseService],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
  ...
  it("Should get share url from server if not cached and use datacontainer routes to set start point", inject(
    [ShareUrlsService, HttpTestingController, DatabaseService],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
  ...
  it("Should get share url from server if not cached and use datacontainer corners to set start point", inject(
    [ShareUrlsService, HttpTestingController, DatabaseService],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
  ...
  it("Should get share url from database when already cached and try to refresh it since it's newer", inject(
    [ShareUrlsService, HttpTestingController, DatabaseService],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
      ...
      databaseService.getShareUrlById = () =>
        Promise.resolve({
          lastModifiedDate: new Date(100).toISOString(),
        } as ShareUrl);
      ...
      mockBackend
        .expectOne(Urls.urls + "7/timestamp")
        .flush(new Date(200).toISOString());

      await new Promise((resolve) => setTimeout(resolve, 100)); // this is in order to let the code continue to run to the next await

      expect(() =>
        mockBackend.expectOne(Urls.urls + "7").flush({
          start: { lat: 1, lng: 1 },
        })
      ).not.toThrow();
      await promise;
  ...
  it("Should get share url from database when already cached and try to refresh twice it since it's newer and first fast refresh failed", inject(
    [ShareUrlsService, HttpTestingController, DatabaseService],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
  ...
  it("Should create share url", inject(
    [ShareUrlsService, HttpTestingController],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController
    ) => {
  ...
  it("Should update share url", inject(
    [ShareUrlsService, HttpTestingController],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController
  ...
  it("Should delete share url", inject(
    [ShareUrlsService, HttpTestingController, DatabaseService, Store],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController,
      databaseService: DatabaseService,
  ...
  it("Should get image for share url", inject(
    [ShareUrlsService],
    (shareUrlsService: ShareUrlsService) => {
  ...
  it("Should set share Url to store", inject(
    [ShareUrlsService, Store],
    (shareUrlsService: ShareUrlsService, store: Store) => {
      store.dispatch = vi.fn();
  ...
  it("Should get share url from store", inject(
    [ShareUrlsService, Store],
    (shareUrlsService: ShareUrlsService, store: Store) => {
      store.reset({
        inMemoryState: {
  ...
  it("Should get users permissions from server", inject(
    [ShareUrlsService, HttpTestingController],
    async (
      shareUrlsService: ShareUrlsService,
      mockBackend: HttpTestingController
  ...
  it("Should get hike icon from hiking type", inject(
    [ShareUrlsService],
    (shareUrlsService: ShareUrlsService) => {
  ...
  it("Should get bike icon from hiking type", inject(
    [ShareUrlsService],
    (shareUrlsService: ShareUrlsService) => {
  ...
  it("Should get 4x4 icon from hiking type", inject(
    [ShareUrlsService],
    (shareUrlsService: ShareUrlsService) => {
  ...
  it("Should get unknown icon from unknown type", inject(
    [ShareUrlsService],
    (shareUrlsService: ShareUrlsService) => {
