...
describe("LayersService", () => {
    ...
    it("should sync user layers when user logs off", inject(
        [LayersService, Store, HttpTestingController],
        async (
    ...
    it("should sync user layers when user logs in with no data", inject(
        [LayersService, Store, HttpTestingController],
        async (
            service: LayersService,
            store: Store,
    ...
    it("should sync user layers when user logs in with data that needs addition, update and removal", inject(
        [LayersService, Store, HttpTestingController],
        async (
            service: LayersService,
            store: Store,
            ...
            backend
                .expectOne((u) => u.url.startsWith(Urls.userLayers))
                .flush([
            ...
            expect(vi.mocked(spy).mock.calls[4][0]).toBeInstanceOf(
                AddOverlayAction
            );
            expect(vi.mocked(spy).mock.calls[5][0]).toBeInstanceOf(
                SetOverlaysVisibilityAction
            );
            expect(vi.mocked(spy).mock.calls[6][0]).toBeInstanceOf(
                RemoveOverlayAction
            );
            expect(vi.mocked(spy).mock.calls[7][0]).toBeInstanceOf(
                RemoveBaseLayerAction
            );
        }
    ));

    it("should check if base layer is selected", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
    ...
    it("should get selected base layer", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
    ...
    it("should return first base layer when selected layer not found", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            store.reset({
                layersState: {
    ...
    it("should return the base layer address when it contains '{x}'", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
    ...
    it("should return default Hebrew tiles when base layer has no address", inject(
        [LayersService, Store, ResourcesService],
        (
            service: LayersService,
            store: Store,
            ...
            (
        resourcesService.getCurrentLanguageCodeSimplified as Mock
            ).mockReturnValue("he");

            const address = service.getSelectedBaseLayerAddressForOSM();

            expect(address).toEqual(
                `${Urls.baseTilesAddress}/Hebrew/Tiles/{z}/{x}/{y}.png`
            );
        }
    ));

    it("should return English tiles when language is not Hebrew", inject(
        [LayersService, Store, ResourcesService],
        (
            service: LayersService,
            store: Store,
        ...
        resourcesService.getCurrentLanguageCodeSimplified as Mock
            ...
            expect(address).toEqual(
                `${Urls.baseTilesAddress}/English/Tiles/{z}/{x}/{y}.png`
            );
        }
    ));

    it("should return MTB tiles for ilMTB style", inject(
        [LayersService, Store, ResourcesService],
        (
            service: LayersService,
            store: Store,
        ...
        resourcesService.getCurrentLanguageCodeSimplified as Mock
            ...
            expect(address).toEqual(
                `${Urls.baseTilesAddress}/Hebrew/mtbTiles/{z}/{x}/{y}.png`
            );
        }
    ));

    it("should check if name is available", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            const layer1: EditableLayer = { key: "layer1" } as EditableLayer;
            const layer2: EditableLayer = { key: "layer2" } as EditableLayer;
    ...
    it("should select base layer", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            const spy = vi.spyOn(store, "dispatch");
    ...
    it("should toggle overlay", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            const spy = vi.spyOn(store, "dispatch");
            store.reset({
    ...
    it("should check if all overlays are hidden", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            const overlay1: EditableLayer = { key: "overlay1" } as EditableLayer;
    ...
    it("should hide all overlays", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            vi.spyOn(store, "dispatch");
            const overlay1: EditableLayer = { key: "overlay1" } as EditableLayer;
    ...
    it("should get data container with selected layers", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
    ...
    it("should add base layer for non logged-in user", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            const spy = vi.spyOn(store, "dispatch");
            const layerData = {
    ...
    it("should add base layer for logged-in user", inject(
        [LayersService, Store, HttpTestingController],
        async (
            service: LayersService,
            store: Store,
            ...
            backend
                .expectOne(
                    (u) => u.method == "GET" && u.url.startsWith(Urls.userLayers)
                )
                .flush(null);
            const spy = vi.spyOn(store, "dispatch");
            const layerData = {
            ...
            backend
                .expectOne(
                    (u) => u.method == "POST" && u.url.startsWith(Urls.userLayers)
                )
                .flush({ id: "1" });
    ...
    it("should not add base layer if it already exists", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            vi.spyOn(store, "dispatch");
    ...
    it("should add overlay for non logged-in user", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            store.reset({
                layersState: {
    ...
    it("should not add overlay if it already exists", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            vi.spyOn(store, "dispatch");
    ...
    it("should update base layer and select it", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            const spy = vi.spyOn(store, "dispatch");
    ...
    it("should update overlay when user is logged in", inject(
        [LayersService, Store, HttpTestingController],
        (service: LayersService, store: Store, backend: HttpTestingController) => {
            store.reset({
                userState: {
                    userInfo: {},
                },
            ...
            backend
                .expectOne(
                    (u) => u.method == "PUT" && u.url.startsWith(Urls.userLayers)
                )
                .flush(null);
            expect(vi.mocked(spy).mock.calls[0][0]).toBeInstanceOf(
                UpdateOverlayAction
    ...
    it("should remove base layer and select first layer", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            const spy = vi.spyOn(store, "dispatch");
    ...
    it("should remove overlay for non registered user", inject(
        [LayersService, Store],
        (service: LayersService, store: Store) => {
            store.reset({
                layersState: {
    ...
    it("should create url from overlay layer data and parse it", inject(
        [LayersService, Store],
        async (service: LayersService, store: Store) => {
            const spy = vi.spyOn(store, "dispatch");
            store.reset({
                layersState: {
                    overlays: [],
            ...
            await service.addLayerAfterNavigation(
            ...
            expect(
                (vi.mocked(spy).mock.calls[0][0] as AddOverlayAction).layerData.key
            ).toBe(layerData.key);
            expect(
                (vi.mocked(spy).mock.calls[0][0] as AddOverlayAction).layerData
                    .address
            ).toBe(layerData.address);
            expect(
                (vi.mocked(spy).mock.calls[0][0] as AddOverlayAction).layerData
                    .minZoom
            ).toBe(layerData.minZoom);
            expect(
                (vi.mocked(spy).mock.calls[0][0] as AddOverlayAction).layerData
                    .maxZoom
            ).toBe(layerData.maxZoom);
            expect(
                (vi.mocked(spy).mock.calls[0][0] as AddOverlayAction).layerData
                    .opacity
            ).toBe(layerData.opacity);
        }
    ));

    it("should create url from base layer data and parse it", inject(
        [LayersService, Store],
        async (service: LayersService, store: Store) => {
            const spy = vi.spyOn(store, "dispatch");
            const layerData: LayerData = {
            ...
            expect(
                (vi.mocked(spy).mock.calls[0][0] as AddBaseLayerAction).layerData
                    .key
            ).toBe(layerData.key);
            expect(
                (vi.mocked(spy).mock.calls[0][0] as AddBaseLayerAction).layerData
