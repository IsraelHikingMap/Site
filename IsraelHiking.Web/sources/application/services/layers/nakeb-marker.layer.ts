    ...
    private fetchMarkers() {
        ...
        this.http.get(url).toPromise().then((response) => {
            ...
            for (let item of data) {
                let marker = L.marker(L.latLng(parseFloat(item.start.lat), parseFloat(item.start.lng)), { draggable: false, clickable: true, keyboard: false, icon: this.markerIcon, title: item.title } as L.MarkerOptions) as Common.IMarkerWithTitle;
