    ...
    export abstract class RouteStateEdit extends RouteStateBase {
        ...
        private dragPointEnd = (marker: L.Marker) => {
            ...
            if (this.selectedRouteSegmentIndex === 0) {
                ...
                chain = chain.then(() => this.context.elevationProvider.updateHeights(this.context.route.segments[0].latlngs)) as angular.IPromise<any>;
            }
        ...
        protected runRouting = (startIndex: number, endIndex: number): angular.IPromise<any> => {
            ...
            promise.then((data) => {
                ...
                deferred.resolve(this.context.elevationProvider.updateHeights(this.context.route.segments[endIndex].latlngs));
            });
