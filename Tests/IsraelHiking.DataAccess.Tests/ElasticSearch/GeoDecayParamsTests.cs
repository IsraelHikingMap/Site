...
public class GeoDecayParamsTests
{
    ...
    [TestMethod]
    public void ComputeGeoDecayParams_ScaleIsTheTunedConstantAcrossZooms()
    {
        foreach (var zoom in new double?[] { null, 0, 5, 8, 11, 14, 18, 22 })
        {
            Assert.AreEqual(8.0, ElasticSearchGateway.ComputeGeoDecayParams(zoom).scaleKm, 1e-9,
                $"scaleKm is a deliberate constant; zoom adaptivity lives in offsetKm (zoom={zoom})");
        }
    }
}
