...
public class GpxDataContainerConverterTests
{
    ...
    public void ToGpx_WithData_ShouldReturnFullGpx()
    {
        var dataContainer = new DataContainerPoco
        {
            ...
            [
                ...
                {
                    Name = "name",
                    Markers = [new MarkerData { Latlng = new LatLng { Lat = 1, Lng = 2 } }],
                    Segments =
                    [
                        new RouteSegmentData
                            { Latlngs = [new LatLngTime { Lat = 3, Lng = 4, Alt = 5 }] }
                    ]
                }
            ]
        };
    ...
    public void ToDataContainer_RouteWithPoints_ShouldReturnRouteWithPointsDataContainer()
    {
        ...
        gpx.Routes.Add(new GpxRoute().WithWaypoints([new GpxWaypoint(new GpxLongitude(1), new GpxLatitude(2), 3)]));
    ...
    public void ToDataContainer_TrackOnlyWithSinglePoint_ShouldReturnEmptyContainer()
    {
        ...
        gpx.Tracks.Add(new GpxTrack().WithSegments([
            ..new[]
            {
                new GpxTrackSegment(new ImmutableGpxWaypointTable([new GpxWaypoint(new GpxLongitude(1), new GpxLatitude(2), 3)
                ]), null)
            }
        ]));
        var dataContainer = _converter.ToDataContainer(gpx);
